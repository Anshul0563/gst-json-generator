...
All output flows from merged_rows list where each row has:
  - invoice_no, pos, txval, igst_amt, cgst_amt, sgst_amt, txn_type
  - supplier_etin (for CLTTX grouping)
  - supplier_name (for logging)

This merged list is used by gst_builder.py to generate:
  - b2cs (grouped by POS)
  - summary (totals from all rows)
  - clttx (grouped by supplier ETIN)
"""
