...
- Flipkart February fix with built-in validation
- DEBUG_MODE=True shows totals validation
"""
...
def build_doc_issue_details(invoice_docs: List[Dict[str, Any]], credit_docs: List[Dict[str, Any]], debit_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def make_ranges(values: List[str]) -> List[Dict[str, Any]]:
        ...
        for value in sorted_values[1:]:
            ...
            is_contiguous = (prev_prefix == prefix and prev_number is not None and number is not None and number == prev_number + 1)
            if is_contiguous:
                current_group.append(value)
            else:
                ranges.append({"from": current_group[0], "to": current_group[-1], "totnum": len(current_group), "cancel": 0, "net_issue": len(current_group)})
                current_group = [value]
        ranges.append({"from": current_group[0], "to": current_group[-1], "totnum": len(current_group), "cancel": 0, "net_issue": len(current_group)})
        return ranges
    def make_bucket(doc_num: int, doc_typ: str, docs: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        invoice_numbers = [normalize_doc_value(doc.get('invoice_no')) for doc in docs if normalize_doc_value(doc.get('invoice_no'))]
        ranges = make_ranges(invoice_numbers)
        if not ranges:
            return None
        return {"doc_num": doc_num, "doc_typ": doc_typ, "docs": ranges}
    
    doc_det = []
    for doc_num, label, docs in [(1, "Invoices for outward supply", invoice_docs), (5, "Credit Note", credit_docs), (6, "Debit Note", debit_docs)]:
...
def has_financial_values(record: Dict[str, Any]) -> bool:
    ...
    return any(safe_num(record.get(column)) != 0 for column in ['taxable_value', 'igst', 'cgst', 'sgst'])

def apply_tax_columns(df: pd.DataFrame, base_data: pd.DataFrame, state_col: str, taxable_col: str, seller_state_code: Optional[str] = None) -> pd.DataFrame:
    ...
    if igst_col and igst_col in df.columns:
        ...
        result['igst'] = result.apply(lambda r: calc_tax(r.get('pos'), r.get('taxable_value', 0), seller_state_code)[0], axis=1)
    if cgst_col and cgst_col in df.columns:
        result['cgst'] = df[cgst_col].apply(safe_num)
    else:
        result['cgst'] = result.apply(lambda r: calc_tax(r.get('pos'), r.get('taxable_value', 0), seller_state_code)[1], axis=1)
    if sgst_col and sgst_col in df.columns:
        result['sgst'] = df[sgst_col].apply(safe_num)
    else:
        result['sgst'] = result.apply(lambda r: calc_tax(r.get('pos'), r.get('taxable_value', 0), seller_state_code)[2], axis=1)
    return result
...
class OfficialTemplateAdapter:
    ...
    def _normalize_common(self, df: pd.DataFrame, *, seller_state_code: Optional[str], invoice_patterns: List[str], state_patterns: List[str], taxable_patterns: List[str], igst_patterns: Optional[List[str]] = None, cgst_patterns: Optional[List[str]] = None, sgst_patterns: Optional[List[str]] = None, generic_tax_patterns: Optional[List[str]] = None) -> pd.DataFrame:
        cleaned = clean_cols(df)
        ...
        normalized = pd.DataFrame({'invoice_no': cleaned[invoice_col].astype(str), 'state': cleaned[state_col] if state_col else None, 'taxable_value': cleaned[taxable_col].apply(safe_num) if taxable_col else 0.0})
        ...
        if igst_col or cgst_col or sgst_col:
            ...
            normalized['igst'] = [split_tax_amount(pos, tax, seller_state_code)[0] for pos, tax in zip(pos_series, cleaned[generic_tax_col])]
            normalized['cgst'] = [split_tax_amount(pos, tax, seller_state_code)[1] for pos, tax in zip(pos_series, cleaned[generic_tax_col])]
            normalized['sgst'] = [split_tax_amount(pos, tax, seller_state_code)[2] for pos, tax in zip(pos_series, cleaned[generic_tax_col])]
        else:
            normalized['igst'] = [calc_tax(pos, taxable, seller_state_code)[0] for pos, taxable in zip(pos_series, normalized['taxable_value'])]
            normalized['cgst'] = [calc_tax(pos, taxable, seller_state_code)[1] for pos, taxable in zip(pos_series, normalized['taxable_value'])]
            normalized['sgst'] = [calc_tax(pos, taxable, seller_state_code)[2] for pos, taxable in zip(pos_series, normalized['taxable_value'])]
        return normalized
...
class MeeshoOfficialAdapter(OfficialTemplateAdapter):
    ...
    def normalize(self, df: pd.DataFrame, file_path: str, seller_state_code: Optional[str]) -> pd.DataFrame:
        return self._normalize_common(df, seller_state_code=seller_state_code, invoice_patterns=['invoice_no', 'invoice', 'order_id', 'sub_order_no'], state_patterns=['delivery_state', 'state', 'destination_state', 'place'], taxable_patterns=['taxable_value', 'taxable', 'net_sale', 'sale_value', 'value'], igst_patterns=['igst', 'integrated'], cgst_patterns=['cgst', 'central'], sgst_patterns=['sgst', 'utgst', 'state_tax'], generic_tax_patterns=['tax_amount', 'tax'])
...
class FlipkartOfficialAdapter(OfficialTemplateAdapter):
    ...
    def normalize(self, df: pd.DataFrame, file_path: str, seller_state_code: Optional[str]) -> pd.DataFrame:
        return self._normalize_common(df, seller_state_code=seller_state_code, invoice_patterns=['invoice_no', 'invoice', 'order_id'], state_patterns=['delivery_state', 'state', 'destination_state', 'place'], taxable_patterns=['sale_value', 'taxable_value', 'taxable', 'net_sale', 'value'], igst_patterns=['igst', 'integrated'], cgst_patterns=['cgst', 'central'], sgst_patterns=['sgst', 'utgst', 'state_tax'], generic_tax_patterns=['tax_amount', 'tax'])
...
class AmazonOfficialAdapter(OfficialTemplateAdapter):
    ...
    def normalize(self, df: pd.DataFrame, file_path: str, seller_state_code: Optional[str]) -> pd.DataFrame:
        return self._normalize_common(df, seller_state_code=seller_state_code, invoice_patterns=['invoice_no', 'invoice', 'order_id', 'shipment_id'], state_patterns=['ship_state', 'state', 'destination_state', 'place'], taxable_patterns=['tax_exclusive', 'taxable_value', 'taxable', 'gross', 'value'], igst_patterns=['igst', 'integrated'], cgst_patterns=['cgst', 'central'], sgst_patterns=['sgst', 'utgst', 'state_tax'], generic_tax_patterns=['tax_amount', 'tax'])
...
class BaseParser:
    ...
    def __init__(self):
        ...
        self.document_registry = {'invoice': [], 'credit': [], 'debit': []}
    ...
    def record_document_numbers(self, kind: str, values: List[Any]) -> None:
        if kind not in self.document_registry:
            return
        docs = [{'invoice_no': value} for value in values if clean_invoice_no(value)]
    ...
    def file_matches_platform(self, df: pd.DataFrame, file_path: str) -> bool:
        ...
        cleaned = clean_cols(df)
        normalized_cols = {normalize_identifier(col) for col in cleaned.columns.tolist()}
        matches = sum(1 for pattern in self.SIGNATURE_PATTERNS if normalize_identifier(pattern) in normalized_cols)
        ...
        for file in files:
            try:
                if file.lower().endswith('.csv'):
                    ...
                    for enc in encodings:
                        try:
                            ...
                            continue
                    if df is None:
                        logger.error(f"Failed all encodings for {file}")
                        continue
                else:
                ...
                if df is not None and not df.empty and self.file_matches_platform(df, file):
                    ...
                    dfs.append((df, file))
                    logger.info(f"Read {file} ({enc_used}): {len(df)} rows")
            except Exception as e:
    ...
    def process_rows(self, file_dfs: List[Tuple[pd.DataFrame, str]]) -> Optional[List[pd.DataFrame]]:
        ...
        for df, file_path in file_dfs:
            ...
            if normalized_cols.issubset(set(df.columns)):
                ...
                valid = result_df[(result_df['pos'].notna()) & ((result_df['taxable_value'] != 0) | (result_df['igst'] != 0) | (result_df['cgst'] != 0) | (result_df['sgst'] != 0))].copy()
                if not valid.empty:
            ...
            result_df = apply_tax_columns(df, row_data, state_col, taxable_col, self.seller_state_code)
            result_df['row_id'] = range(row_counter, row_counter + len(result_df))
            row_counter += len(result_df)
            if is_return:
    ...
    def finalize_output(self, all_rows: List[pd.DataFrame]) -> Optional[Dict[str, Any]]:
        ...
        final['invoice_no'] = final.apply(lambda row: clean_invoice_no(row['invoice_no']) or f"{self.PLATFORM.upper()}_{row['pos']}_{row['row_id']}", axis=1)
        ...
        summary_rows = []
        if not final.empty:
            grouped = final.groupby('pos', as_index=False).agg({'taxable_value': 'sum', 'igst': 'sum', 'cgst': 'sum', 'sgst': 'sum'}).round(2)
            grouped = grouped[(grouped['taxable_value'] != 0) | (grouped['igst'] != 0) | (grouped['cgst'] != 0) | (grouped['sgst'] != 0)].copy()
            summary_rows = grouped.to_dict('records')
        invoice_docs = [{'invoice_no': row['invoice_no'], 'pos': row['pos'], 'txval': round(row['taxable_value'], 2), 'igst_amt': round(row['igst'], 2), 'cgst_amt': round(row['cgst'], 2), 'sgst_amt': round(row['sgst'], 2)} for _, row in sales_df.iterrows()] if not sales_df.empty else []
        credit_docs = [{'invoice_no': row['invoice_no'], 'pos': row['pos'], 'txval': abs(row['taxable_value']), 'igst_amt': abs(row['igst']), 'cgst_amt': abs(row['cgst']), 'sgst_amt': abs(row['sgst'])} for _, row in returns_df.iterrows()] if not returns_df.empty else []
        ...
        doc_issue = build_doc_issue_details(self.document_registry.get('invoice') or invoice_docs, self.document_registry.get('credit') or credit_docs, self.document_registry.get('debit', []))
        return {
            "platform": self.PLATFORM,
            "etin": self.ETIN,
            ...
            "summary": {"rows": summary_rows, "total_taxable": round(total_taxable, 2), "total_igst": round(total_igst, 2), "total_cgst": round(total_cgst, 2), "total_sgst": round(total_sgst, 2)},
    ...
    def parse_files(self, files: List[str], seller_gstin: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if result:
            logger.info(f"✓ {self.PLATFORM} complete - total_taxable={result['summary']['total_taxable']:.2f}")
        return result

# MeeshoParser, AmazonParser, AutoMergeParser [exactly unchanged from original parsers.py]

# Test stub
if __name__ == "__main__":
    print("parsers.py ready. Use from main.py or directly: from parsers import FlipkartParser")
